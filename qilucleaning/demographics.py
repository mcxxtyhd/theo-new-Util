from qilucleaning.model import BR_Demographics
...
def cleaningBR_Demographics():
    ...
    # 如果数据为0了就不要再循环了
    while result_num>0:
        all_results = conntheo.theo.session.query(BR_Demographics).order_by(BR_Demographics.BR_Demographics_ID).slice(numFirst, numSecond).all()
