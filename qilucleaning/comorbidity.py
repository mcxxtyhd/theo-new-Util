from qilucleaning.model import BR_Comorbidity
from qilucleaning.sqlconnnection import connsql
from utils.EncryptUtils import pyEOR

# 既往病史的加密
def cleaningcomorbidity():

    # 初始化连接
    conntheo = connsql()

    print('****开始 comorbidity ****')
    ...
    # 如果数据为0了就不要再循环了
    while result_num>0:
        all_results = conntheo.theo.session.query(BR_Comorbidity).order_by(BR_Comorbidity.BR_ComorbidityID).slice(numFirst, numSecond).all()
